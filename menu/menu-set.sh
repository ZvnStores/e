...
echo -e "     ${BICyan}[${BIWhite}10${BICyan}] Change NameServer Slowdns"
echo -e "     ${BICyan}[${BIWhite}11${BICyan}] Renew Pubkey & Private Key Slowdns"

echo -e " ${BICyan}└─────────────────────────────────────────────────────┘${NC}"
echo -e "     ${BIYellow}Press x or [ Ctrl+C ] • To-${BIWhite}Exit${NC}"
echo ""
read -p " Select Options [ 1-11 or x ] : " opt
echo -e ""
case $opt in
...
9) clear ; bw ; exit ;;
10) clear ; nsmenu ; exit ;;
11) clear ; pubkey ; exit ;;
0) clear ; menu ; exit ;;
x) exit ;;
